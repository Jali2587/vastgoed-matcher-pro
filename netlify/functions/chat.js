...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.body.prepend(uploadInput);
        ...
        Papa.parse(file, {
            ...
            complete: function(results) {
                ...
                results.data.forEach((row, index) => {
                    const firstName = row['First Name'] || row['Voornaam'] || '';
                    const lastName = row['Last Name'] || row['Achternaam'] || '';
                    const email = row['Email Address'] || row['E-mailadres'] || '';
                    const title = row['Job Title'] || row['Functietitel'] || '';

                    if (!firstName && !lastName) return;

                    const docRef = db.collection('contacts').doc();
                    const type = title.toLowerCase().includes('broker') ? 'broker' :
                                 title.toLowerCase().includes('investeerder') ? 'investor' : 'contact';
                ...
                batch.commit()
                    .then(() => alert(`✅ ${savedCount} contacten succesvol geüpload!`))
